describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const prodItems = await page.$$('product-item');
    const firstItems = prodItems[0];
    const shadowR = await firstItems.getProperty('shadowRoot');
    const shadowButton = await shadowR.$('button');
    await shadowButton.click();
    const buttonText = await shadowButton.getProperty('innerText');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const prodItems = await page.$$('product-item');
    const testItems = prodItems.slice(1,);
    for (let i of testItems) {
      const shadowR = await i.getProperty('shadowRoot');
      const shadowButton = await shadowR.$('button');
      await shadowButton.click();
    }
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    const localCart = await page.evaluate(() => localStorage.cart);
